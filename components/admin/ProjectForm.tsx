...
function GalleryEditor({
  ...
  const update  = (i: number, url: string) => { const a = [...images]; a[i] = url; onChange(a); };
        ...
        {images.map((img, i) => (
          <div key={i} className="border border-gray-200 rounded-lg p-3 relative">
            <button
              type="button"
              onClick={() => remove(i)}
              className="absolute top-2 right-2 w-6 h-6 bg-red-100 hover:bg-red-500 text-red-600 hover:text-white rounded-full flex items-center justify-center transition-colors z-10"
            >
              <X className="w-3 h-3" />
            </button>
            <ImageUpload value={img} onChange={url => update(i, url)} label={`Image ${i + 1}`} />
          </div>
        ))}

        {images.length === 0 && (
          <div className="border-2 border-dashed border-gray-200 rounded-lg py-8 flex flex-col items-center gap-2 text-gray-400">
