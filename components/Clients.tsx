...
export default function Clients({ clients, testimonials }: { clients: Client[]; testimonials: Testimonial[] }) {
  ...
  return (
    <section id="clients" className="py-20 bg-white" style={banglaFont}>
      <div className="container mx-auto px-4 lg:px-8">
        <div className="text-center mb-14">
          <p className="section-subtitle mb-3">{tr.subtitle}</p>
          <h2 className="section-title mb-4">{tr.title}</h2>
          <p className="text-gray-500 max-w-xl mx-auto">{tr.desc}</p>
        </div>
        ...
        <div className="text-center mb-12">
          <p className="section-subtitle mb-3">{tr.reviewsSub}</p>
          <h2 className="section-title">{tr.reviewsTitle}</h2>
        </div>

        <div className="grid md:grid-cols-3 gap-6">
          {testimonials.map((t_item) => {
            const name = (lang === 'bn' && t_item.name_bn) ? t_item.name_bn : t_item.name;
            const role = (lang === 'bn' && t_item.role_bn) ? t_item.role_bn : t_item.role;
            const text = (lang === 'bn' && t_item.text_bn) ? t_item.text_bn : t_item.text;
            return (
              <div key={t_item.id} className="card p-6 relative">
                <Quote className="absolute top-4 right-4 w-8 h-8 text-primary-100" />
                <div className="flex gap-1 mb-4">
                  {Array.from({ length: t_item.rating }).map((_, i) => <Star key={i} className="w-4 h-4 fill-amber-400 text-amber-400" />)}
                </div>
                ...
                <div className="flex items-center gap-3 pt-4 border-t border-gray-100">
                  <div className="w-10 h-10 rounded-full bg-primary-600 flex items-center justify-center text-white font-bold text-sm flex-shrink-0">
                    {t_item.name.split(' ').map((n) => n[0]).slice(0, 2).join('')}
                  </div>
                  <div>
